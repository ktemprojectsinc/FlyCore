...
public class TestClass extends Ktem_Module  { //Наследовать класс модуля
	...
	public void enableModule(){//Включение модуля
		...
		this.enableThreads();//Регистрация таймеров
		this.newListener((Listener)new TestListener());//Регистрация Listener
	}
	
	private void enableThreads(){//Метод создания таймеров
		this.newThread("TestThread",//Название нового Thread, если уже существует не будет создан
				(60*60),//Время в секундах, в данный момент в час
				()->{//То что будет выполнятся
					Core.sendOnlinePlayers((gamer)->!gamer.isDonater(),//Проверка игрока, может ли он получить действие, в данный момент донатер или нет
							(gamer,p)->{//То что делать с теми кто может получить
								gamer.playSound(Ktem_Sound.LEVEL_UP);//Звук
								p.sendTitle("","§fКупи донат на §bwww.flycore.ru",15,40,20);//Титл 1 строчка/Титл 2 строчка/время появления/Время держания/Время скрытия
								gamer.sendMessages(Arrays.asList("",
									"§a§lFlyCore §8| §fПокупай донат только на сайте §bwww.flycore.ru§f и не где больше!",""));//Сообщение игроку
							});
				});
	}
